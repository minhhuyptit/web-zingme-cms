...
if (isset($_GET['pageNum_rs_theloaitin'])) {
  $pageNum_rs_theloaitin = $_GET['pageNum_rs_theloaitin'];
}
...
<section class="phantrang">
       <p class="boxes">
        <?php
        $TFM_Previous = $pageNum_rs_theloaitin - 5;
        if ($TFM_Previous >= 0) {
        ...
        for ($i = $TFM_startLink; $i <= $TFM_endLink; $i++) {
          ...
          if($TFM_LimitPageEndCount != $pageNum_rs_theloaitin) {
            printf('<a href="'."%s?pageNum_rs_theloaitin=%d%s", $currentPage, $TFM_LimitPageEndCount, $queryString_rs_theloaitin.'">');
            echo "$i</a>";
          }else{
        ...
        ?>
        <?php
        $TFM_Next = $pageNum_rs_theloaitin + 5;
        $TFM_Last = $totalPages_rs_theloaitin+1;
        if ($TFM_Next - 1 < $totalPages_rs_theloaitin) { 
          printf('...<a href="'."%s?pageNum_rs_theloaitin=%d%s", $currentPage, $TFM_Next, $queryString_rs_theloaitin.'">');
