...
$KTColParam2_rs_tintieudiem = "Phong-su";
if (isset($_GET["keyseo"])) {
  $KTColParam2_rs_tintieudiem = $_GET["keyseo"];
}
mysql_select_db($database_conn_vietchuyen, $conn_vietchuyen);
$query_rs_tintieudiem = sprintf("SELECT theloai.ID_theloai, theloai.tentheloai, theloai.keyword, theloaitin.ID_theloaitin, theloaitin.tentheloaitin, theloaitin.keyseo, tintuc.ID_tintuc, tintuc.tieudetin, tintuc.hinhtrichdan, tintuc.kiemduyet, tintuc.ngaycapnhat, tintuc.ID_phanloaitin FROM ((tintuc LEFT JOIN theloai ON theloai.ID_theloai=tintuc.ID_theloai) LEFT JOIN theloaitin ON theloaitin.ID_theloaitin=tintuc.ID_theloaitin) WHERE theloai.keyword=%s  AND theloaitin.keyseo=%s  AND tintuc.kiemduyet=1  AND tintuc.ID_phanloaitin=1 ORDER BY tintuc.ngaycapnhat DESC LIMIT 0,5", GetSQLValueString($KTColParam1_rs_tintieudiem, "text"),GetSQLValueString($KTColParam2_rs_tintieudiem, "text"));
...
<?php if ($totalRows_rs_tintieudiem > 0) { // Show if recordset not empty ?>
  <section class="tintieudiem">
    ...
    <article class="<?=($count == $totalRows_rs_tintieudiem) ? "tintieudiem_baiviet_cuoi":"tintieudiem_baiviet"?>"> 
    	<a href="<?=$url?>a<?=$row_rs_tintieudiem['ID_theloai']; ?>/b<?=$row_rs_tintieudiem['ID_theloaitin']; ?>/c<?=$row_rs_tintieudiem['ID_tintuc']; ?>/<?=vietdecode($row_rs_tintieudiem['tieudetin']);?>.html">
        	<img class="canhlechohinh" src="<?=$url?><?php echo $objDynamicThumb1->Execute(); ?>" border="0" alt="<?php echo $row_rs_tintieudiem['tieudetin']; ?>" />
        </a>
      <h1>
      	<span class="linkden">
        	<a href="<?=$url?>a<?=$row_rs_tintieudiem['ID_theloai']; ?>/b<?=$row_rs_tintieudiem['ID_theloaitin']; ?>/c<?=$row_rs_tintieudiem['ID_tintuc']; ?>/<?=vietdecode($row_rs_tintieudiem['tieudetin']);?>.html"><?php echo $row_rs_tintieudiem['tieudetin']; ?></a>

            <!--<a href="layout_chitiettin.php?cat=<?php echo $row_rs_tintieudiem['ID_theloai']; ?>&subcat=<?php echo $row_rs_tintieudiem['ID_theloaitin']; ?>&id=<?php echo $row_rs_tintieudiem['ID_tintuc']; ?>"><?php echo $row_rs_tintieudiem['tieudetin']; ?></a>
-->        </span>
      </h1>
