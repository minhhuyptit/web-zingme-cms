...
if (isset($_GET['pageNum_rs_theloai'])) {
  $pageNum_rs_theloai = $_GET['pageNum_rs_theloai']-1;
}
...
$keyword=$row_rs_theloai['keyword'];
...
?>
    ...
    <section class="phantrang">
       <p class="boxes">
        <?php
        $TFM_Previous = $pageNum_rs_theloai - 4;
        if ($TFM_Previous >= 1) {
           //printf('...<a href="'."%s?pageNum_rs_theloai=%d%s", $currentPage, $TFM_Previous, $queryString_rs_theloai.'">');
		   printf('...<a href="'."%s%d/%s", $currentPage, $TFM_Previous, $keyword.'.html">');
    ...
    <?php
        for ($i = $TFM_startLink; $i <= $TFM_endLink; $i++) {
          ...
          if($TFM_LimitPageEndCount != $pageNum_rs_theloai) {
            //printf('<a href="'."%s?pageNum_rs_theloai=%d%s", $currentPage, $hocwebgiare, $queryString_rs_theloai.'">');
			printf('<a href="'."%s%d/%s", $currentPage, $hocwebgiare, $keyword.'.html">');
            echo "$i</a>";
          }else{
        ...
        ?>
        <?php
        $TFM_Next = $pageNum_rs_theloai + 6;
        $TFM_Last = $totalPages_rs_theloai+1;
        if ($TFM_Next - 1 < $totalPages_rs_theloai+1) { 
          //printf('...<a href="'."%s?pageNum_rs_theloai=%d%s", $currentPage, $TFM_Next, $queryString_rs_theloai.'">');
		  printf('...<a href="'."%s%d/%s", $currentPage, $TFM_Next, $keyword.'.html">');
